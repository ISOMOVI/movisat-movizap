...
class TestReagir:
    def test_reage_e_grava(self, cena, sem_whatsapp):
        ...
        assert banco.um("SELECT reacao FROM mensagem WHERE id = %s",
                        (cena["msg"],))["reacao"] == "👍"
...
class TestEncaminhar:
    ...
    def test_encaminhar_ARQUIVO_ainda_nao(self, cena, sem_whatsapp):
        """⚠️ Diz o que falta em vez de mandar só a legenda e deixar o outro
        lado sem o anexo -- que seria pior do que recusar."""
        midia = banco.um(
            """INSERT INTO midia (conversa_id, caminho, mime, tamanho)
               VALUES (%s, '/tmp/zz', 'image/png', 10) RETURNING id""",
            (cena["c1"],))["id"]
        com_arquivo = banco.um(
            ...
            (cena["c1"], midia))["id"]
        r = conversas.encaminhar(com_arquivo, [cena["c2"]], cena["eu"])
        assert r["ok"] is False
        assert "arquivo" in r["motivo"].lower()
        banco.executar("DELETE FROM mensagem WHERE id = %s", (com_arquivo,))
